...
var platform_resource = [load(path_large_p),-1]
...
var chunk_item_spawn = 2
#if item is spawned
var item_spawned = false
#path to load for item
var item_resource = load("res://Resources_Scenes/Inventory/ItemDrop/ItemLightning/ItemLightning.tscn")
#item to load in the world
var item

var slime_resources = [load("res://Resources_Scenes/SlimeWater/Slime.tscn"),"slime",0]
...
func _ready():
	...
	if chunk_number > 1:
		terrain_resources[3].x = get_node("../"+str(chunk_number-1)+"/"+str(chunk_number-1)+str(8)).position.x + terrain_resources[2]
...
func create_chunk():
	create_base_terrain()
	last_unit = terrain[-1].position.x + terrain_resources[2]/2
...
func normalize_platform(var platform_input):
	if platform_input.position.x + half_large.x > last_unit:
		return platform_input.position.x + half_large.x - last_unit
	else:
		return 0
func spawn_platform(var p):
	...
	for n in p:
		...
		if spawn_enemy > 0 and chunk_number > 3:
			print("entro")
			rng.randomize()
			spawn = rng.randi_range(spawn_prob,2)
			if spawn == 2:
				spawn_prob = 1
			else:
				spawn_prob += 1
...
func get_position():
	...
	if first_spawn and chunk_number == 1:
		rng.randomize()
		position.x = terrain[1].position.x + half_large.x +  rng.randi_range(range_x_y_lvl_one_f[0]/2,range_x_y_lvl_one_f[1]/2)
		rng.randomize()
		position.y = terrain[1].position.y - half_large.y +  rng.randi_range(range_x_y_lvl_one_f[2],range_x_y_lvl_one_f[3])
		first_spawn = false
	elif first_spawn:
		first_spawn = false
		...
		if current_p_from_spawn.position.y > -50:
			chance_down = 1
		rng.randomize()
		position.x = current_p_from_spawn.position.x + half_large.x + rng.randi_range(range_x_y_lvl_one_f[0],range_x_y_lvl_one_f[1])
		rng.randomize()
