...
#path of the small platform
var path_small_p = "res://Resources_Scenes/FlyingPlatformSmall/floatingPlatform.tscn"
#path of the large platform
var path_boss_p = "res://Resources_Scenes/FlyingPlatformBoss/floatingPlatform.tscn"
#reasouces for the boss
var boss_resource = [load(path_boss_p)]
#platform to load for the boss
var boss_platform
#loaded path of all the platform, plus the current name of the platform
var platform_resource = [load(path_medium_p),-1, load(path_small_p)]
...
var chunk_item_spawn = 5
#if item is spawned
var item_spawned = false
#path to load for item
var item_resource = load("res://Resources_Scenes/Inventory/ItemDrop/ItemWater/ItemWater.tscn")
#item to load in the world
var item

var slime_resources = [load("res://Resources_Scenes/SlimeFire/Slime.tscn"),"slime",0]
...
func _ready():
	...
	spawn_boss = get_node("../").spawn_boss
	print("spawn boss",spawn_boss)
	...
	for n in p:
		...
		if chunk_number % 2 == 0:
			if !munition_spawned:
				...
				if platform[-1].type == "small":
					munition.position.x -= 2
				else:
					munition.position.x -= 5
				platform[-1].add_child(munition)
		self.add_child(platform[-1])
		if chunk_number == chunk_item_spawn and !item_spawned: #chunk_item_spawn
			item_spawned = true
...
func create_platform_boss():
	print("lastplatform", platform[-1].name)
	var half_x =  Vector2.ZERO
	var chance_down = 1
	if platform[-1].type == "medium":
		half_x = half_x_medium
		...
		half_x = half_x_small
	boss_platform = boss_resource[0].instance()
	if platform[-1].position.y < -140:
			chance_down = -1.5
	if platform[-1].position.y > -50:
		chance_down = 1
	rng.randomize()
	boss_platform.position.x = platform[-1].position.x + half_x.x + rng.randi_range(range_x_y_lvl_one_f[0],range_x_y_lvl_one_f[1])
	rng.randomize()
	boss_platform.position.y = platform[-1].position.y + rng.randi_range(range_x_y_lvl_one_f[2],range_x_y_lvl_one_f[3]) * chance_down
	boss_platform.name = "boss_platform"
	self.add_child(boss_platform)
